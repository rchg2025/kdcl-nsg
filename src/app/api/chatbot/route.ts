      ...
      for (const cri of std.criteria) {
        ...
        if (cri.evidences && cri.evidences.length > 0) {
          ...
          for (const ev of cri.evidences) {
            ...
            try {
              if (ev.fileUrl) {
                const arr = JSON.parse(ev.fileUrl);
                if (Array.isArray(arr) && arr.length > 0 && arr[0].url) {
                  parsedFileUrl = arr[0].url;
                } else if (typeof ev.fileUrl === 'string' && ev.fileUrl.startsWith('http')) {
                  parsedFileUrl = ev.fileUrl;
                }
              }
            } catch (e) {
              if (ev.fileUrl?.startsWith('http')) parsedFileUrl = ev.fileUrl;
            }

            detailsStr += `      - File/Nội dung: ${ev.content || "Tệp đính kèm"} ${parsedFileUrl ? `(Link tài liệu: ${parsedFileUrl})` : ''} | Trạng thái: ${ev.status} | Người nộp: ${ev.collaborator?.name || "Ẩn danh"}\n`
...
ĐẶC BIỆT QUAN TRỌNG: Khi nhắc đến hoặc liệt kê bất kỳ tài liệu, minh chứng nào có chứa (Link tài liệu: ...), bạn BẮT BUỘC phải tạo liên kết dạng Markdown để người dùng bấm vào xem được tài liệu đó. Đảm bảo url trong ngoặc đơn () là một url hợp lệ. Ví dụ: [Kế hoạch khảo sát](https://drive.google.com/...). Nếu không có (Link tài liệu: ...), đừng tự bịa link.
