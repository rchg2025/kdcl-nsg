...
import { createDepartment, deleteDepartment, createPosition, deletePosition } from "@/actions/category"
import { Plus, Trash2, Building2, Briefcase, Loader2 } from "lucide-react"
      ...
      <div className="glass rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800">
        ...
        <div className="space-y-2">
          {departments.length === 0 ? (
            ...
            departments.map(d => (
              <div key={d.id} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl group hover:border-blue-200 border border-transparent transition-colors">
                <span className="font-medium text-sm text-slate-700 dark:text-slate-300">{d.name}</span>
                <button onClick={() => handleDeleteDept(d.id)} className="text-slate-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity p-1">
                  <Trash2 size={16} />
                </button>
              </div>
            ))
      ...
      <div className="glass rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800">
        ...
        <div className="space-y-2">
          {positions.length === 0 ? (
            ...
            positions.map(p => (
              <div key={p.id} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl group hover:border-amber-200 border border-transparent transition-colors">
                <span className="font-medium text-sm text-slate-700 dark:text-slate-300">{p.name}</span>
                <button onClick={() => handleDeletePos(p.id)} className="text-slate-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity p-1">
                  <Trash2 size={16} />
                </button>
              </div>
            ))
