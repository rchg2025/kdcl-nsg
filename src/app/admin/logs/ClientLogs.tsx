...
export default function ClientLogs({ logs }: { logs: LogItem[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return result
  }, [logs, filterKeyword, dateFrom, dateTo])
      ...
      <div className="glass rounded-xl p-4 border border-slate-200 dark:border-slate-800 shadow-sm relative z-30">
        <div className="flex flex-col md:flex-row gap-4 md:items-end">
            ...
            <div className="relative">
              ...
              <datalist id="usersList">
                {uniqueUsers.map(([email, name]) => (
                  <option key={email} value={`${name} - ${email}`} />
                ))}
              </datalist>
            </div>
          </div>
          ...
          <div className="flex w-full md:w-auto gap-2 justify-end">
            <button
              onClick={() => { setFilterKeyword(""); setDateFrom(""); setDateTo(""); resetPage() }}
              className="flex-1 md:flex-none px-4 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg text-sm font-medium transition-colors justify-center flex items-center gap-1.5"
            >
        ...
        <div className="mt-3 text-xs text-slate-400">
          ...
          {(filterKeyword || dateFrom || dateTo) && " (đã lọc)"}
